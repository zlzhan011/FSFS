...
if __name__ == '__main__':
    c_root = ''
    COMPAS_path = r'E:\code\competitor\Complexity-Driven-Feature-Construction\Finding-Fair-Representations-Through-Feature-Construction\data\compas-analysis'
    c_root = COMPAS_path
    df = pd.read_csv(COMPAS_path + '\compas-scores-two-years.csv')

    df = df.drop('id', axis=1)
    from analysis_compas.compas_process import compas_preprocess

    df = df.sample(frac=1, random_state=42)

    df = df[['age', 'c_charge_degree', 'race', 'age_cat', 'score_text', 'sex', 'priors_count',
             'days_b_screening_arrest', 'decile_score', 'c_jail_in', 'c_jail_out','is_recid']]
    df = df.dropna()
    ...
    for kfold_i, (train_indices, test_indices) in enumerate(kfold.split(df)):
        ...
        selectedFeatures, redundancy_feature, independent_feature, time22 = osfs_z_mb(traindata, class_index, 0.01, max_k=100)
        ...
        workbook.save(os.path.join(c_root, 'analysis_EO_LR_v3_'+str(kfold_i)+".xlsx"))
