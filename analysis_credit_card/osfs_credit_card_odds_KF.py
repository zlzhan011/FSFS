...
from analysis_german.model import modeling_no_knn
...
def find_admissible_features(y_mb, y_redundancy, p_mb, p_redundancy, p_independent, p_index, sheet, cell_number):
    ...
    if len(intersection) == 0:
        return list(y_mb), list(y_mb), sheet
    else:
...
if __name__ == '__main__':
    ...
    c_root = r'E:\Research\Dataset\Collect_dataset\credit_card'
    file_path = os.path.join(c_root, 'default of credit card clients.xls')
    from analysis_credit_card.credit_card_preprocess import load_credit_card_v2
    df = load_credit_card_v2(file_path)
    ...
    for kfold_i, (train_indices, test_indices) in enumerate(kfold.split(df)):
        ...
        time_end = time.time()
        print("process data cost time:", time_end - time_start)


        import time
        time_start = time.time()
        modeling_res = modeling_no_knn(X_train, y_train, X_test, y_test, write_flag='all_features', c_root=c_root)
        ...
        time_end = time.time()
        print("all feature  cost time:", time_end - time_start)


        traindata = np.concatenate((X_train, y_train), axis=1)
        class_index = traindata.shape[-1] - 1
        # selectedFeatures, time =osfs_d(traindata,class_index,0.01,'chi2')
        # one_res['selectedFeatures osfs_d'] = selectedFeatures
        # selectedFeatures, redundancy_feature, independent_feature, time = osfs_d_mb(traindata, class_index, 0.01, 'g2')
        ...
        time_start = time.time()
        modeling_res_y_mb = modeling_no_knn(X_train[:,selectedFeatures], y_train, X_test[:,selectedFeatures], y_test, write_flag='y_mb', c_root=c_root)
        ...
        for si in range(len(sensitive_index_list)):
            ...
            modeling_res_y_mb_rs = modeling_no_knn(X_train[:, feature_remove_s], y_train, X_test[:, feature_remove_s], y_test,
                                            write_flag='y_mb', c_root=c_root)
            y_test_predict_y_mb_z_rs = modeling_res_y_mb_rs['MLP']['y_pre']
            y_test_predict_y_mb_z_lgbm_rs = modeling_res_y_mb_rs['LGBM']['y_pre']
            rs_mlp_acc = str(modeling_res_y_mb_rs['MLP']['accuracy'])
            rs_lgbm_acc = str(modeling_res_y_mb_rs['LGBM']['accuracy'])
            ...
            if isinstance(selectedFeatures_y_label, list):
                pass
            else:
                selectedFeatures_y_label = selectedFeatures_y_label.tolist()
            if sensitive_index in selectedFeatures_y_label:
                selectedFeatures_y_label.remove(sensitive_index)
            ...
            import time
            time_start = time.time()
            modeling_res_y_mb_remove_i = modeling_no_knn(X_train[:, selectedFeatures], y_train, X_test[:, selectedFeatures],
                                                  y_test,
                                                  write_flag='y_mb', c_root=c_root)
            ...
            # # add admissible features
            print("&&&&&&&&&add admissible features&&&&&&&&&&&&&")
            if admissible_i == admissible_r:
                print("==================admissible_i is equla to admissible_r")
                print("admissible_i:", admissible_i)
                ...
                print("admissible_i == admissible_r after add admissible features discrimination :",
                ...
                modeling_res_y_mb_add_a = modeling_no_knn(X_train[:, selectedFeatures], y_train, X_test[:, selectedFeatures], y_test,
                                             write_flag='y_mb', c_root=c_root)
                ...
                time_end = time.time()
                print("ad1  cost time:", time_end - time_start)

                # y_test_predict, acc_test_admissible_i = MLP_Keras_V2(X_train_processed, y_train,
                ...
                # y_test_predict_xgb, acc_test_admissible_i_xgb = XGB(X_train_processed, y_train,
                #                                                     X_test_processed, y_test)
                ...
                print("admissible_i:", admissible_r)
                one_res_s['i not equal r admissible_r'] = admissible_r
                selectedFeatures = admissible_r
                if sensitive_index in selectedFeatures:
                    selectedFeatures.remove(sensitive_index)
                sheet['F' + str(cell_number + 27)] = str(selectedFeatures)
                # y_test_predict, acc_test_admissible_r = MLP_Keras_V2(X_train[:, selectedFeatures], y_train,
                #                                                      X_test[:, selectedFeatures], y_test)
                # one_res_s['acc_test_i_not_equal_r_admissible_r'] = acc_test_admissible_r
                time_start = time.time()
                modeling_res_y_mb_add_a_r = modeling_no_knn(X_train[:, selectedFeatures], y_train, X_test[:, selectedFeatures],
                                                     y_test,
                                                     write_flag='y_mb', c_root=c_root)
                ...
                time_end = time.time()
                print("ad2  cost time:", time_end - time_start)
                original_discrimination = calculate_original_discrimination(class_label=y_test_predict,
                                                                            discrimination_column=X_test[:,
                                                                                                  sensitive_index])
                odds_difference = equalized_odds_difference(y_test,
            ...
            workbook.save(os.path.join(c_root, 'analysis_v2_'+str(kfold_i)+".xlsx"))
